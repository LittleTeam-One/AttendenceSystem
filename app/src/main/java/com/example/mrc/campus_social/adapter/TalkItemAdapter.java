...
public class TalkItemAdapter extends RecyclerView.Adapter<TalkItemAdapter.ViewHolder>{
    ...
    public TalkItemAdapter(String tag, Context context, Map<Integer, CommunityFragment.Post> data) {
        mTag = tag;
        mData = data;
        mContext = context;
    }
    ...
    public TalkItemAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = View.inflate(mContext, R.layout.card_item_mood_talk, null);
        return new TalkItemAdapter.ViewHolder(view);
    }
    ...
    public void onBindViewHolder(TalkItemAdapter.ViewHolder holder, int position) {
        CommunityFragment.Post p = mData.get(position + 1);
        holder.mSubTitle.setText(p.getSubTitle());
        holder.mAutor.setText(p.getName());
